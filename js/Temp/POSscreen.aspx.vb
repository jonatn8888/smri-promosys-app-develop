...
Partial Class POSscreen
    ...
    Private Function GetFolderNames(ByVal flag As Int32) As DataTable
        ...
        With sqlCmd
            .CommandText = "select FolderName from tbl_POS where AlreadyUploaded = '" & flag.ToString.Trim & "'"
            .Connection = sqlConn
            .CommandTimeout = 0
            .CommandType = 1
        End With
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Request("DbName") <> Nothing Then
            Dim fname As String
            fname = Left(POSNetworkDrive, Len(POSNetworkDrive) - 1) & Session("FolderParent").ToString & "/" & Request("DbName")
...
#Region "Grid View"
    Protected Sub GVChild_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs)
        If e.CommandName = "Print" Then
            ...
            Session("FolderParent") = "/" & Left(lnk2.Text, Len(lnk2.Text) - 4)

            lbl = GVChild.Rows(CInt(lnk1.Text) - 1).FindControl("LinkButton3")

            Response.Redirect("POSscreen.aspx?DbName=" & lbl.Text)
    ...
    Protected Sub GridView1_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridView1.RowCommand
        If e.CommandName = "Download" Then
            ...
            Session("FolderParent") = ""
            clsPromo.CreateZipFile(POSNetworkDrive & Dt.Rows(e.CommandArgument)(0), 0)
            Session("row") = e.CommandArgument

            'ViewState("newfoldername") = UpdateFolderName(Left(lbl.Text.Trim, Len(lbl.Text.Trim) - 4))
            ...
            Response.Redirect("POSscreen.aspx?DbName=" & Dt.Rows(e.CommandArgument)(0))
        ...
        ElseIf e.CommandName = "Preview" Then
            ...
            If lnkAdd.Text = "+" Then
                ...
                Dim lbl As LinkButton
                lbl = GridView1.Rows(e.CommandArgument).FindControl("lblUploadDate")
                Session("FolderParent") = "/" & Left(lbl.Text, Len(lbl.Text) - 4)
                Me.GridView1.Rows(e.CommandArgument).FindControl("GVChild").Visible = True
                ''Me.GridView1.Rows(e.CommandArgument).FindControl("gvMemo").Visible = True
            ...
            Else
                ...
                ''Me.GridView1.Rows(e.CommandArgument).FindControl("gvMemo").Visible = False
                CType(GridView1.Rows(e.CommandArgument).FindControl("lnkAdd"), LinkButton).Text = "+"
            End If
        Else
            Me.GridView1.Rows(e.CommandArgument).FindControl("GVChild").Visible = False
    ...
    Private Sub fillMainTable()
        ...
        a = My.Computer.FileSystem.GetDirectories(POSNetworkDrive)

        For Each path As String In a

            dr = Dt.NewRow()
            d = Split(path, "\")

            For Each row As DataRow In dtable.Rows
                If (row.Item(0)) = d(UBound(d)) Then
                    dr("UploadDate") = d(UBound(d)) & ".zip"
                    Dt.Rows.Add(dr)
                    aa = aa + 1
                End If
            Next
    ...
    Private Sub mids()
        ...
        For Each path As String In a
            ...
            For Each row As DataRow In dtable.Rows
                If (row.Item(0)) = d(UBound(d)) Then
                    fillChildTable(path, aa, d(UBound(d)))
                    fillAnotherChildTable(aa, d(UBound(d)))
                    aa = aa + 1
                End If
            Next
        Next
...
#End Region
    ...
    Private Function getPromoApproved(ByVal param As String) As DataTable
        ...
        Dim sqlCmd As New SqlClient.SqlCommand("exec USP_SelectPromoApproved '" & param & "'", sqlConn)
    ...
    Private Sub UpdatePOSFLAG(ByVal foldername As String, ByVal param2 As String)
        Dim sqlConn As SqlClient.SqlConnection
        sqlConn = New SqlClient.SqlConnection(clsPromo.SQLConnString())
        Dim sqlCmd As New SqlClient.SqlCommand("exec USP_UpdatePOSFLAG  '" & foldername & "','" & param2 & "'", sqlConn)
        sqlCmd.CommandType = CommandType.Text
        Dim da As New SqlClient.SqlDataAdapter(sqlCmd)
        Dim ds As New DataSet
        Try
