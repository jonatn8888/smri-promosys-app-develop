...
Partial Class ViewMemoSinglePromo
    ...
    Private Sub LoadMemoInformation()
        ...
        Else
            ...
            With rowMemos
                ...
                'Dim dv As DataView
                'dv = sqldsPromos.Select(DataSourceSelectArguments.Empty)
                'Dim dr As DataRow
                'dr = dv.Table.Rows(0)
                'Try
                '    lblDeptSdept.Text = dr("ItemCode")
                'Catch ex As Exception
                '    lblDeptSdept.Text = Nothing
                'End Try
                'lblPromoType.Text = dr("TypeDesc")

                ViewState("RequestID") = CInt(.RequestID)

                strQuery = "SELECT TypeDesc FROM Promotions P " & _
                            "INNER JOIN PromoTypes PT ON p.PromoTypeID = pt.PromoTypeID " & _
                            "WHERE P.RequestID = 0" & ViewState("RequestID")

                If clsSystemApp.GetDataRow(clsPromo.SQLConnString, strQuery, drPromoType) Then
                    lblPromoType.Text = drPromoType("TypeDesc")
                End If
        ...
        If System.IO.Directory.Exists(strDir) Then
            ...
            For Each file As System.IO.FileInfo In files
                ...
                strFiles = strFiles & file.Name.ToString & ","
                 ' Revised dowcarpio03142013@smretailinc: reported error in viewing attachment Ticket # 413215
                lblFiles.Text &= "<img src='Images/bullet green.gif' /><a href=""ViewMemoSinglePromo.aspx?FileName=" & Server.UrlEncode(clsPromo.pathAttachment & ViewState("RequestID") & "\" & file.Name.ToString) & "&MemoID=" & clsSession.CurrMemoID.ToString & """>" & file.Name.ToString & "</a> "
            Next
        End If
